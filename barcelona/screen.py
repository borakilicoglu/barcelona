...
class Application:
    ...
    def get_header_right(self):
        return Panel(Align.center(f"Squad value\n[b]{self.scrapper.team_value}[/b]", vertical="middle"))

    def get_main_left_top(self):
        standings = Table(expand=True, box=box.SIMPLE)
        standings.add_column(header="#")
        standings.add_column(header="Clubs")
        standings.add_column(header="Matches")
        standings.add_column(header="+/-")
        standings.add_column(header="Point")
        ...
        return Panel(standings, title="[cyan][b]TABLE STANDING LA LIGA[/b][/cyan]", box=box.SQUARE)
    ...
    def get_main_left_bottom(self):
        ...
        truths.add_row("Club name", ": ", tr["legal_name"])
        truths.add_row("Address", ": ", tr["address"])
        truths.add_row("Tel", ": ", tr["telephone"])
        truths.add_row("Fax", ": ", tr["fax"])
        truths.add_row("Website", ": ", tr["url"])
        truths.add_row("Founded", ": ", tr["founded"])
        truths.add_row("Members", ": ", tr["members"])
        return Panel(truths, title="[cyan][b]INFORMATION[/b][cyan]", box=box.SQUARE)
    ...
    def get_main_middle(self):
        ...
        return Panel(
            Panel(
                Group(*items),
                title="[cyan][b]NEXT MATCHES[/b][/cyan]"
            ),
            box=box.SIMPLE
        )
    ...
    def get_main_right(self):
        ...
        for each in self.scrapper.rumors[:4]:
            ...
            rumors.add_row("Age", ": ", str(each["player"]["age"]))
            rumors.add_row("Position", ": ", each["player"]["position"])
            rumors.add_row("Market value", ": ", each["player"]["marketValue"])
        ...
        return Panel(
            Panel(
                Group(*items),
                title="[cyan][b]CURRENT RUMORS[/b][/cyan]"
            ),
            box=box.SIMPLE
        )
    ...
    def get_footer():
        ...
        table.add_row("credit", ": ", Text("@borakilicoglu", style="link https://github.com/borakilicoglu"))
...
class Footballers:
    ...
    def get_main(self):
        ...
        return Panel(standings, title="[cyan][b]Barcelona Squad[/b][/cyan]", box=box.SQUARE)
